    ...
    try {
      ...
      if (state !== 'completed' && state !== 'failed' && !(await isBotWorkerAlive())) {
        ...
        res.write(`data: ${JSON.stringify({
          ...
          totalInvoicesExpected: progressData.totalInvoicesExpected ?? null,
          outputCount: progressData.outputCount ?? 0,
          inputCount: progressData.inputCount ?? 0,
