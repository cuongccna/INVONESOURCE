/**
 * backend/scripts/seed.ts — Development seed data
...
async function seed(): Promise<void> {
  ...
  try {
    ...
    await client.query(
      `INSERT INTO users (id, email, password_hash, full_name) VALUES ($1,$2,$3,$4) ON CONFLICT DO NOTHING`,
    ...
    await client.query(
      ...
      [adminId1, company1Id, 'OWNER']
    );
    await client.query(
      `INSERT INTO user_companies (user_id, company_id, role) VALUES ($1,$2,$3) ON CONFLICT DO NOTHING`,
      [adminId2, company2Id, 'OWNER']
    );
    await client.query(
      `INSERT INTO user_companies (user_id, company_id, role) VALUES ($1,$2,$3) ON CONFLICT DO NOTHING`,
      [accountantId, company1Id, 'ACCOUNTANT']
    );
    ...
    for (let i = 0; i < 80; i++) {
      ...
      await client.query(
        `INSERT INTO invoices (
           id, company_id, provider, external_id, invoice_number, serial_number,
         ...
         ON CONFLICT (company_id, provider, invoice_number, seller_tax_code, invoice_date) DO NOTHING`,
        [
    ...
    await client.query(
      `INSERT INTO vat_reconciliations
         (id, company_id, period_month, period_year, output_vat, input_vat, payable_vat, breakdown, generated_at)
       VALUES ($1,$2,$3,$4,$5,$6,$7,$8,NOW()) ON CONFLICT DO NOTHING`,
      [uuidv4(), company1Id, now.getMonth() + 1, now.getFullYear(),
       500_000_000, 300_000_000, 200_000_000, JSON.stringify({ by_rate: {} })]
    );
    ...
    console.log('\n✅ Seed completed!');
    console.log('  Admin 1: admin@demoabc.vn / Admin@123456');
    console.log('  Admin 2: admin@xyz.com.vn / Admin@123456');
