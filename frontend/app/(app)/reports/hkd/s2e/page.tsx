...
interface CashSection { rows: CashRow[]; total_in: number; total_out: number; }
interface S2eData {
...
export default function S2ePage() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      const res = await api.get('/hkd-reports/s2e', { params: { quarter, year } });
        ...
        <table className="w-full text-sm">
          ...
          <tfoot>
            <tr className="bg-gray-50 font-bold border-t-2 border-gray-200">
              ...
              <td className="px-3 py-2 text-right tabular-nums text-green-800">{fmt(section.total_in)}</td>
              <td className="px-3 py-2 text-right tabular-nums text-red-800">{fmt(section.total_out)}</td>
            </tr>
          </tfoot>
        </table>
      </div>
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 py-6 space-y-4">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
        ...
        <button
          onClick={() => downloadExcel(`/hkd-reports/s2e/excel?quarter=${quarter}&year=${year}`, `S2e-HKD_Q${quarter}_${year}.xlsx`)}
