 ...
 *
 * ═════════════════════════════════════════════════════════════════════════════
 * TABLE DELETION SUMMARY
 * ═════════════════════════════════════════════════════════════════════════════
 *
 * ✅ CASCADE TABLES (auto-deleted when companies.id is deleted):
 *    - user_companies, company_connectors, company_settings
 *    - invoices (with invoice_line_items, gdt_bot_configs cascading)
 *    - tax_declarations, tax_declaration_attachments, vat_reconciliations
 *    - sync_logs, notifications
 *    - customer_rfm, payment_plan_assignments, price_alerts
 *    - product_catalog, customer_catalog, supplier_catalog
 *    - inventory_movements, cash_book_entries
 *    - telegram_chat_configs, sequential_numbering
 *    - hkd_declarations, hkd_line_items
 *    - company_analytics, esg_estimates, insights_cache
 *    - repurchase_predictions, price_anomalies, audit_rule_configs
 *    - code_sequences, company_risk_flags, raw_invoice_data
 *    - vendor_blacklist, import_sessions, import_templates
 *
 * 🔴 NO CASCADE TABLES (must be manually deleted):
 *    - gdt_bot_runs: company_id UUID NOT NULL (no FK constraint at all)
 *    - import_temp_files: company_id UUID NOT NULL (no FK constraint at all)
 *    - bot_failed_jobs: company_id UUID REFERENCES companies(id) WITHOUT CASCADE
 *
 * 🟡 INDIRECT CASCADE (deleted via invoice cascade):
 *    - gdt_validation_queue: invoice_id FK ON DELETE CASCADE
 *    - invoice_detail_queue: invoice_id FK ON DELETE CASCADE
 *
 * � AUDIT LOGS (preserved for tracking - not deleted):
 *    - audit_logs: company_id REFERENCES companies(id) - kept for audit trail
 *
 * �🟢 SET NULL (not deleted, just nullified):
 *    - companies.parent_id (hierarchy self-reference)
 *    - licenses.company_id (can exist without company)
 *    - gdt_raw_cache.company_id (cache can exist without company)
 *    - static_proxy_pool.company_id (proxy pool can exist without company)
 * ═════════════════════════════════════════════════════════════════════════════
 */
...
async function main() {
  ...
  const deleteSteps = [
    // ── Tables with NO FK or NO CASCADE on company_id (must delete explicitly) ──
    ...
    // bot_failed_jobs has company_id UUID REFERENCES companies(id) WITHOUT CASCADE
    { name: 'bot_failed_jobs',     sql: 'DELETE FROM bot_failed_jobs   WHERE company_id = $1' },
    
    // Note: audit_logs is intentionally NOT deleted to preserve audit trail/history

    // ── Tables linked via invoices (cascade from invoices deletion) ──
    // gdt_validation_queue has invoice_id FK with ON DELETE CASCADE
    ...
    // invoice_detail_queue has invoice_id FK with ON DELETE CASCADE
    { name: 'invoice_detail_queue (via invoices)', sql: `
        DELETE FROM invoice_detail_queue
        WHERE invoice_id IN (SELECT id FROM invoices WHERE company_id = $1)
      ` },

    // ── CASCADE deletion of company (cascades to all other tables) ──
